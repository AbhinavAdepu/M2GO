...
-(void)MainDescription
{
	...
	if ([txtMobileNumber.text length] > 0)
	{
        ...
        tmpstr=[tmpstr stringByAppendingFormat:@"SMSTO:%@",txtMobileNumber.text];
	...
	if ([txtMessage.text length] > 0)
	{
		...
		str = [str stringByAppendingFormat:@"\n\n"];
         tmpstr=[tmpstr stringByAppendingFormat:@":%@",txtMessage.text];
	}
    if ([txtMobileNumber.text length] == 0 && [txtMessage.text length] > 0)
	{
        tmpstr=@"";
         tmpstr=[tmpstr stringByAppendingFormat:@"SMSTO::%@",txtMessage.text];
    }
